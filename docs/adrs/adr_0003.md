...
| Capture storage layers list index/vector only (no Git Notes tier) | `src/services/capture.rs` | L16-L22 | gap |

**Summary:** Feature flags support tiering, but Tier 1 references Git Notes persistence which is no longer present.

**Action Required:** Update tier definitions to reflect current storage or reintroduce Tier 1 persistence mapping.
