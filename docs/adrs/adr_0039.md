...
| Status command does not report migration state | `src/cli/status.rs` | L1-L17 | gap |

**Summary:** Embeddings are optional and manual migration exists, but status output does not surface migration state.

**Action Required:** Expose migration status in the status command or update ADR expectations.
