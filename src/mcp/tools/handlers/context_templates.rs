...
use crate::mcp::tool_types::{
    ...
    ContextTemplateRenderArgs, ContextTemplateSaveArgs, TemplatesArgs, domain_scope_to_display,
    parse_domain_scope,
};
...
// =============================================================================
// Consolidated Context Template Handler
// =============================================================================
...
fn execute_templates_save(args: &TemplatesArgs) -> Result<ToolResult> {
    let name = args
        .name
        .as_ref()
    ...
    let content = args
        .content
        .as_ref()
        .ok_or_else(|| Error::InvalidInput("'content' is required for save action".to_string()))?;
    ...
    // Build template
    let mut template = ContextTemplate::new(name, content);
    template.output_format = output_format;
    ...
    if let Some(tags) = &args.tags {
        template = template.with_tags(tags.clone());
    }
    ...
    if let Some(vars) = &args.variables_def {
        let variables: Vec<TemplateVariable> = vars
            ...
            .map(|v| {
                let mut tv = TemplateVariable::new(&v.name);
                if let Some(desc) = &v.description {
                    tv = tv.with_description(desc.clone());
                }
                if let Some(default) = &v.default {
                    tv = tv.with_default(default.clone());
                }
                // Set required - with_default sets required to false
                if v.required.unwrap_or(true) && tv.default.is_none() {
                    tv.required = true;
    ...
    // Create service and save
    let mut service = ContextTemplateService::new();
    let (saved_name, version) = service.save(&template, domain)?;
...
fn execute_templates_list(args: &TemplatesArgs) -> Result<ToolResult> {
    // Parse domain scope
    let domain = args.domain.as_deref().map(|s| parse_domain_scope(Some(s)));
    ...
    if let Some(limit) = args.limit {
        filter = filter.with_limit(limit.min(100) as usize);
    } else {
        filter = filter.with_limit(20);
    }
...
fn execute_templates_get(args: &TemplatesArgs) -> Result<ToolResult> {
    let name = args
        .name
        .as_ref()
    ...
    // Create service and get
    let mut service = ContextTemplateService::new();
    let template = service.get(name, args.version, domain)?;
...
fn execute_templates_render(args: &TemplatesArgs) -> Result<ToolResult> {
    let name = args
        .name
        .as_ref()
        .ok_or_else(|| Error::InvalidInput("'name' is required for render action".to_string()))?;
    ...
    // Get custom variables
    let custom_vars = args.variables.clone().unwrap_or_default();
...
fn execute_templates_delete(args: &TemplatesArgs) -> Result<ToolResult> {
    let name = args
        .name
        .as_ref()
    ...
    // Create service and delete
    let mut service = ContextTemplateService::new();
    let deleted = service.delete(name, args.version, domain)?;
